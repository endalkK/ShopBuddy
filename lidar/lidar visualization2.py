...
#https://github.com/makertut/raspberry-rplidar/blob/master/sample.py
...
def _process_scan(raw):
    ...
    if new_scan == inversed_new_scan:
        ...
        print("RPLidarException('New scan flags mismatch')")
    check_bit = raw[1] & 0b1
    if check_bit != 1:
        #raise RPLidarException('Check bit not equal to 1')
        print("RPLidarException('Check bit not equal to 1')")
...
def lidar_measurments(self, max_buf_meas=500):
        ...
        if dsize != 5:
            ...
            print("RPLidarException('Wrong info reply length')")
        if is_single:
            #raise RPLidarException('Not a multiple response mode')
            print("RPLidarException('Not a multiple response mode')")
        if dtype != SCAN_TYPE:
            #raise RPLidarException('Wrong response data type')
            print("RPLidarException('Wrong response data type')")
        while True:
